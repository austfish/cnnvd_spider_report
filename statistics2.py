...
def up_type(date):
    try:
        year_id = get_year()
        search_result = ES2.search(index='te-type', doc_type='te-type', q='_id:"%s"' % year_id)
        # 判断数据是否存在
...
def up_level(date):
    try:
        year_id = get_year()
        search_result = ES2.search(index='te-level', doc_type='te-level', q='_id:"%s"' % year_id)
        # 判断数据是否存在
        if search_result['hits']['hits']:
            result = ES2.update(
                index='te-level',
...
def up_total(date):
    try:
        year_id = get_year()
        search_result = ES2.search(index='te-total', doc_type='te-total', q='_id:"%s"' % year_id)
        # 判断数据是否存在
        if search_result['hits']['hits']:
            result = ES2.update(
                index='te-total',
...
def up_vendor(date):
    try:
        year_id = get_year()
        search_result = ES2.search(index='te-vendor', doc_type='te-vendor', q='_id:"%s"' % year_id)
        # 判断数据是否存在
        if search_result['hits']['hits']:
            result = ES2.update(
                index='te-vendor',
...
def up_all(date):
    try:
        year_id = get_year()
        search_result = ES2.search(index='te-stat', doc_type='te-stat', q='_id:"%s"' % year_id)
        # 判断数据是否存在
        if search_result['hits']['hits']:
            result = ES2.update(
                index='te-stat',
